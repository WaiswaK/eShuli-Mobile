                            ...
                            foreach (var topic in topics)
                            {
                                try
                                {
                                    db.Insert(new Topic()
                                    {
                                        ...
                                        teacher_full_names = topic.teacher,
                                        TopicTitle = topic.TopicTitle,
                                        Updated_at = topic.Updated_at,
                                        Folder_Id = topic.folder_id,
                                        Folder_Name = topic.folder_name });
                                }
                                catch
                                {
            ...
            try
            {
                ...
                foreach (var subject in subjects)
                {
                    ...
                    if (topics != null)
                    {
                        foreach (var topic in topics)
                        {
                            Topic newTopic = new Topic(topic.TopicID, subject.Id, topic.TopicTitle, topic.overview, topic.body, topic.Updated_at, topic.teacher, topic.folder_id, topic.folder_name);
                            try
                            {
                                db.Update(newTopic);
                            }
                    ...
                    if (assignments != null)
                    {
                        foreach (var assignment in assignments)
                        {
                            ...
                            List<AttachmentModel> oldfiles = DBRetrievalTask.OldGetFiles(assignment.id);
